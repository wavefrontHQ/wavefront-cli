...
class Install(Base):
    ...
    def run(self):
        ...
        if prompt:
            proxy_input = input("Would you like to install the Wavefront Proxy on this host? (yes or no) [default: no]: \n").lower()
            if proxy_input == "y" or proxy_input == "yes":
                proxy = True
            else:
                proxy = False

            agent_input = input("Would you like to install Telegraf (metric collection agent) on this host? (yes or no) [default: no]: \n").lower()
            if agent_input == "y" or agent_input == "yes":
                agent = True
                agent_tags = input('Please enter a comma separated list of tags you would like added to metrics from this Telegraf host ("env=dev,app=myapp") or press Enter to continue without adding any tags: \n')
            else:
                agent = False
            statsd_input = input("Would you like to configure StatsD integration on this host? (yes or no) [default: no]: \n").lower()
            ...
            if lib.aws.is_ec2_instance():
                aws_input = input("Would you like to add AWS EC2 Metadata to metrics from this host? (yes or no) [default: no]: \n").lower()
        ...
        if proxy:
            if not wavefront_url:
                wavefront_url = input("Please enter the url to your Wavefront instance (default = https://try.wavefront.com): \n") or "https://try.wavefront.com"
            if not api_token:
                api_token = input("Please enter a valid Wavefront API Token: \n")
        ...
        if agent:
            ...
            if not proxy_address:
                proxy_address = input("Please enter the address to your Wavefront proxy (default = localhost): \n") or "localhost"
            if not proxy_port:
                proxy_port = input("Please enter the port of your Wavefront proxy (default = 2878): \n") or "2878"
        ...
        if aws:
            if not aws_region:
                aws_region = input("Please enter the AWS region (example = us-west-2): \n")
            if not aws_secret_key_id:
                aws_secret_key_id = input("Please enter your AWS IAM Access Key ID: \n")
            if not aws_secret_key:
                aws_secret_key = input("Please enter your AWS IAM Secret Key: \n")
        ...
        if statsd:
            if not statsd_port:
                statsd_port = input("Please enter the port you would like StatsD to listen on (default = 8125): \n") or "8125"
